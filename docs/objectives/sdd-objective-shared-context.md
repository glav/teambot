...
@builder-1 Can you implement the high priority recommendations frrom $pm
```
...
The status of @reviewer will be reflected as performing the task while the 'builder-1 agent will reflect as waiting until it begins to act on the output of 'pm' agent when it will be reflected as performing the task.

**Success Criteria**:
- [ ] Must provide an easy to use syntax to reference another agents results or output like '$pm' for the 'pm' agents results.
- [ ] If a prompt references another agents resuts, then the agent on which references the other agent will wait for that agent to complete its output then perform the action using those latest results.
