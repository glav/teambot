...
### [ ] Phase 1: Worktree Module Foundation (TDD)
...
- [ ] `src/teambot/worktree/` module exists with `__init__.py`, `errors.py`, `manager.py`
- [ ] Validation: `uv run pytest tests/test_worktree/ -v` passes
- [ ] Artifacts: 95%+ coverage on worktree module
...
- [ ] `teambot run obj.md --worktree` parses correctly
- [ ] Validation: `uv run pytest tests/test_cli.py -k worktree -v` passes
- [ ] Artifacts: CLI integration tests passing

**Cannot Proceed If**: `--worktree` flag not recognized by argument parser

---

### [ ] Phase 3: UI Indicators (TDD)
...
- [ ] REPL prompt shows `[wt:feat/branch]` when in worktree
- [ ] Validation: `uv run pytest tests/test_repl/ -k worktree -v` passes
- [ ] Artifacts: UI indicator tests passing
...
- [ ] Path validation catches 260+ char paths on Windows
- [ ] Validation: `uv run pytest tests/test_worktree/test_validation.py -v` passes
- [ ] Artifacts: Error handling tests passing
...
- [ ] Acceptance tests pass with real Git operations
- [ ] Validation: `uv run pytest tests/test_worktree_acceptance.py -v -m acceptance` passes
- [ ] Artifacts: Acceptance test report
...
- [ ] CLI `--help` shows worktree flags with descriptions
- [ ] Validation: `uv run teambot run --help | grep worktree` shows flag
- [ ] Artifacts: docs/guides/worktree-isolation.md exists
