...
- [ ] **Task 1.1**: Write test for `NOTIFICATION_MODES` constant existence and structure
...
- [ ] `uv run pytest tests/test_notifications/test_modes.py` passes
- [ ] Validation: `uv run ruff check src/teambot/notifications/modes.py`
- [ ] Artifacts: `modes.py` exists with `NOTIFICATION_MODES` and `resolve_notification_mode()`
...
- [ ] `uv run pytest tests/test_notifications/test_config.py` passes
- [ ] Validation: `uv run ruff check src/teambot/notifications/config.py`
- [ ] Artifacts: `config.py` updated with mode expansion logic
...
- [ ] Manual test: `teambot init` shows mode selection
- [ ] Generated `teambot.json` includes `notification_mode`
- [ ] Validation: `uv run ruff check src/teambot/cli.py`
...
- [ ] Full notification test suite passes
- [ ] Documentation updated with mode descriptions
...
- [ ] `notification_mode` config option works with all three modes
- [ ] `stages_only` sends exactly 3 event types
- [ ] `agent_status` sends exactly 6 event types
- [ ] `all` sends all events (no filtering)
- [ ] `events` array takes precedence over `notification_mode`
- [ ] Default behavior (neither specified) is `all` events
- [ ] Invalid mode raises `ValueError` with helpful message
- [ ] `teambot init` offers mode selection
- [ ] Existing configurations continue to work unchanged
- [ ] All tests pass: `uv run pytest tests/test_notifications/`
