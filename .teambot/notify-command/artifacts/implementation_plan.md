...
- [ ] `custom_message` events bypass mode-based filtering (`stages_only`, `agent_status`)
- [ ] Explicit `events` arrays continue to take precedence (user can still exclude `custom_message`)
- [ ] No regression in existing `notification_mode` filtering for automated events
- [ ] 100% test coverage for new bypass logic
...
- [ ] **T1.1**: Add `TestCustomMessageBypassMode` test class to `tests/test_notifications/test_config.py`
...
- [ ] All 4 tests added to test file
- [ ] Tests run and **fail** (TDD red phase)
- [ ] Validation: `uv run pytest tests/test_notifications/test_config.py::TestCustomMessageBypassMode -v` shows 4 failures
- [ ] Artifacts: Test class added to `test_config.py`
...
- [ ] **T2.1**: Modify `_create_channel()` in `src/teambot/notifications/config.py`
...
- [ ] Code modification complete in `config.py`
- [ ] No syntax errors: `python -c "from teambot.notifications.config import _create_channel"`
...
- [ ] **T3.2**: Run full notification test suite for regression
  - Command: `uv run pytest tests/test_notifications/ -v`
  - Expected: All existing tests pass

- [ ] **T3.3**: Lint and format code
...
- [ ] Linting passes
- [ ] Validation: `uv run pytest tests/test_notifications/ -v` shows 100% pass
...
- [ ] **T4.1**: Update notification documentation if `docs/guides/notifications.md` exists
...
- [ ] Artifacts: Updated docs (if applicable)
...
- [ ] `@notify` sends with `notification_mode: stages_only` configured
- [ ] `@notify` sends with `notification_mode: agent_status` configured
- [ ] `@notify` blocked when `events: []` explicitly set
- [ ] `notification_mode` filtering unchanged for other event types
- [ ] All notification tests pass (existing + new)
