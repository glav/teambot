...
- [ ] SDK client captures `ASSISTANT_USAGE` events
- [ ] TaskResult has optional `token_usage` field
- [ ] Backward compatibility maintained (old code works)
- [ ] Validation: `uv run pytest tests/test_copilot/ tests/test_tasks/ -v`
...
- [ ] `src/teambot/tokens/display.py` exists
- [ ] `render_token_summary()` and `render_session_summary()` functions work
- [ ] Display shows "n/a" when tokens unavailable
- [ ] Validation: `uv run pytest tests/test_tokens/test_display.py -v`
...
- [ ] Orchestration runs display token summary at end
- [ ] REPL sessions display token summary on exit
- [ ] Token data persists in `workflow_state.json` metadata
- [ ] `token_tracking.enabled` config option works
- [ ] Validation: `uv run teambot run docs/objectives/test-objective.md` (manual verify)
...
* [ ] Task 5.1: Create acceptance tests (AT-001 through AT-006)
...
- [ ] Unit coverage for tokens module ≥90%
