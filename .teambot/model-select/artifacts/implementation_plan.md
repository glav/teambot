...
- [ ] **T1.1T**: Write TDD tests for model validation (Details: Lines 30-65)
- [ ] **T1.1**: Create `VALID_MODELS` constant in `config/schema.py` (Details: Lines 67-95)
- [ ] **T1.2**: Implement `validate_model()` function (Details: Lines 97-125)

### Phase Gate: Phase 1 Complete When
- [ ] All Phase 1 tasks marked complete
- [ ] `uv run pytest tests/test_config/test_schema.py -v` passes
- [ ] Validation: `validate_model("gpt-5")` returns `True`
- [ ] Artifacts: `config/schema.py` with VALID_MODELS and validate_model()
...
- [ ] **T2.1T**: Write TDD tests for config model loading (Details: Lines 130-175)
- [ ] **T2.1**: Extend `ConfigLoader._validate_agent()` for model field (Details: Lines 177-220)
- [ ] **T2.2**: Add `model` field to `create_default_config()` agents (Details: Lines 222-250)

### Phase Gate: Phase 2 Complete When
- [ ] All Phase 2 tasks marked complete
- [ ] `uv run pytest tests/test_config/test_loader.py -v` passes
- [ ] Validation: Invalid model in config raises `ConfigError`
- [ ] Artifacts: Updated `config/loader.py` with model validation
...
- [ ] **T3.1T**: Write TDD tests for AgentStatus.model field (Details: Lines 255-295)
- [ ] **T3.1**: Add `model` field to `AgentStatus` dataclass (Details: Lines 297-335)
- [ ] **T3.2**: Add `model` field to `Task` dataclass (Details: Lines 337-375)

### Phase Gate: Phase 3 Complete When
- [ ] All Phase 3 tasks marked complete
- [ ] `uv run pytest tests/test_ui/test_agent_state.py tests/test_tasks/test_models.py -v` passes
- [ ] Validation: `AgentStatus(agent_id="pm", model="gpt-5")` creates successfully
- [ ] Artifacts: Updated `agent_state.py` and `models.py`
...
- [ ] **T4.1**: Modify `CopilotSDKClient.get_or_create_session()` for model (Details: Lines 380-430)
- [ ] **T4.2**: Implement model resolution logic (inline > session > config > default) (Details: Lines 432-485)
- [ ] **T4.1T**: Write integration tests for SDK model passing (Details: Lines 487-530)

### Phase Gate: Phase 4 Complete When
- [ ] All Phase 4 tasks marked complete
- [ ] `uv run pytest tests/test_copilot/test_sdk_client.py -v` passes
- [ ] Validation: Session config dict contains "model" key
- [ ] Artifacts: Updated `sdk_client.py` with model support
...
- [ ] **T5.1T**: Write TDD tests for `--model` flag parsing (Details: Lines 535-585)
- [ ] **T5.1**: Add `--model`/`-m` flag parsing to `parse_command()` (Details: Lines 587-640)
- [ ] **T5.2**: Add `model` field to `Command` dataclass (Details: Lines 642-670)

### Phase Gate: Phase 5 Complete When
- [ ] All Phase 5 tasks marked complete
- [ ] `uv run pytest tests/test_repl/test_parser.py -v` passes
- [ ] Validation: `@pm --model gpt-5 task` parses with `result.model == "gpt-5"`
- [ ] Artifacts: Updated `parser.py` with model flag support
...
- [ ] **T6.1T**: Write TDD tests for `/models` command (Details: Lines 675-715)
- [ ] **T6.1**: Implement `handle_models()` function (Details: Lines 717-760)
- [ ] **T6.2**: Implement `handle_model()` for session override (Details: Lines 762-820)

### Phase Gate: Phase 6 Complete When
- [ ] All Phase 6 tasks marked complete
- [ ] `uv run pytest tests/test_repl/test_commands.py -v` passes
- [ ] Validation: `/models` lists all 14 valid models
- [ ] Artifacts: Updated `commands.py` with /models and /model handlers
...
- [ ] **T7.1**: Update `StatusPanel._format_status()` to display model (Details: Lines 825-870)
- [ ] **T7.2**: Update `/status` command output format (Details: Lines 872-910)
- [ ] **T7.3**: Update `/tasks` command to show model per task (Details: Lines 912-950)
- [ ] **T7.1T**: Write tests for UI display format (Details: Lines 952-995)

### Phase Gate: Phase 7 Complete When
- [ ] All Phase 7 tasks marked complete
- [ ] `uv run pytest tests/test_ui/ tests/test_visualization/ -v` passes
- [ ] Validation: Visual inspection shows model in status panel
- [ ] Artifacts: Updated `status_panel.py`, `console.py`, command output
...
- [ ] **T8.1**: Integrate model resolution in `TaskExecutor` (Details: Lines 1000-1055)
- [ ] **T8.2**: Update file-based orchestration to use agent models (Details: Lines 1057-1110)
- [ ] **T8.1T**: Write end-to-end integration tests (Details: Lines 1112-1160)

### Phase Gate: Phase 8 Complete When
- [ ] All Phase 8 tasks marked complete
- [ ] Full test suite passes: `uv run pytest --cov=src/teambot`
- [ ] Validation: Manual test with `@pm --model claude-opus-4.5 task`
- [ ] Artifacts: Updated `executor.py`, `execution_loop.py`
