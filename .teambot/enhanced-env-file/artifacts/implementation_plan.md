...
- [ ] All unit tests written in `tests/test_env_loader.py`
- [ ] Tests fail with appropriate "module not found" or "function not found" errors
- [ ] Validation: `uv run pytest tests/test_env_loader.py` runs (failures expected)
- [ ] Artifacts: `tests/test_env_loader.py` exists with 15+ test functions
...
- [ ] Validation: `uv run pytest tests/test_env_loader.py -v` shows all green
- [ ] Artifacts: `src/teambot/env_loader.py` with all functions implemented
...
- [ ] `--env-file` and `--no-env` appear in `teambot --help`
- [ ] Arguments are mutually exclusive (argparse enforces)
- [ ] Validation: `teambot --help | grep -E "(env-file|no-env)"` shows both options
...
- [ ] Validation: `uv run pytest tests/test_cli.py -v -k env` shows all green
- [ ] Artifacts: Extended `tests/test_cli.py` with new test class
...
- [ ] Validation: `uv run pytest tests/test_env_loading_acceptance.py -v -m acceptance`
- [ ] Artifacts: `tests/test_env_loading_acceptance.py` with 8 test functions
...
- [ ] README updated with `--env-file` and `--no-env` documentation
- [ ] All tests pass: `uv run pytest`
- [ ] Coverage meets 85%+ for new module
- [ ] Validation: `uv run pytest --cov=src/teambot/env_loader --cov-report=term-missing`
- [ ] Artifacts: Updated `README.md`
