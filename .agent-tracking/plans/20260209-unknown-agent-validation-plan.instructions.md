...
- [x] After the `CommandType.AGENT` check (line 176) and before routing to sub-methods (line 178), add a validation block
- [x] Collect all agent IDs: from `command.agent_ids` for simple/multi-agent, from `command.pipeline` stages for pipelines
- [x] Resolve each ID via `AGENT_ALIASES.get(agent_id, agent_id)`
- [x] Check each canonical ID against `VALID_AGENTS`
- [x] On first invalid ID, return `ExecutionResult(success=False, output="", error=f"Unknown agent: '{agent_id}'. Valid agents: {valid_list}")`
- [x] Use local import pattern (matches existing pattern at line 199): `from teambot.repl.router import AGENT_ALIASES, VALID_AGENTS`
...
- [x] At the top of `_handle_agent_command()` (after the executor-None check at line 147), add a validation block
- [x] Iterate `command.agent_ids`, resolve aliases, check against `VALID_AGENTS`
- [x] On first invalid ID, call `output.write_task_error(agent_id, error_message)` and return early
- [x] Use same local import: `from teambot.repl.router import AGENT_ALIASES, VALID_AGENTS`
- [x] This prevents `set_running()` and `execute_streaming()` from being called with invalid agents
...
- [x] Validation: `@unknown task &` returns error in executor; `@fake-agent task` returns error in app
...
- [x] In `_update()` (line 184): before auto-creating a new `AgentStatus`, add `if agent_id not in DEFAULT_AGENTS: return`
- [x] In `set_idle()` (line 159): before auto-creating, add `if agent_id not in DEFAULT_AGENTS: return`
- [x] In `set_model()` (line 202): before auto-creating, add `if agent_id not in DEFAULT_AGENTS: return`
- [x] Use existing `DEFAULT_AGENTS` constant (line 73) — no new imports needed
- [x] Do NOT change behavior for agent IDs already present in `_statuses` dict

**Success Criteria**: `manager.set_running("fake", "task")` does not create a new status entry; `manager.get("fake")` returns `None`.

### Phase Gate: Phase 2 Complete When
- [x] Task 2.1 marked complete
- [x] `uv run pytest` passes
- [x] Validation: `AgentStatusManager().set_running("fake", "x"); assert manager.get("fake") is None`
...
- [x] Add new class `TestTaskExecutorAgentValidation`
- [x] `test_execute_rejects_unknown_agent` — `@unknown-agent task &` → `result.success is False`, error contains `"Unknown agent: 'unknown-agent'"`
- [x] `test_execute_rejects_unknown_in_multiagent` — `@pm,fake-agent task` → entire command rejected, SDK not called
- [x] `test_execute_rejects_unknown_in_pipeline` — `@fake -> @pm task` → entire pipeline rejected
- [x] `test_execute_accepts_valid_alias` — `@project_manager task &` → succeeds
- [x] `test_execute_accepts_all_valid_agents` — all 6 canonical IDs accepted (regression guard)
- [x] `test_execute_error_message_lists_valid_agents` — error message contains all 6 valid agents sorted
- [x] `test_execute_rejects_unknown_agent_background` — `@unknown task &` → rejected, no background task created
...
- [x] Add new class `TestAgentStatusManagerGuard`
- [x] `test_set_running_ignores_unknown_agent` — `set_running("fake", "task")` → `get("fake") is None`
- [x] `test_set_idle_ignores_unknown_agent` — `set_idle("fake")` → `get("fake") is None`
- [x] `test_set_model_ignores_unknown_agent` — `set_model("fake", "gpt-4")` → `get("fake") is None`
- [x] `test_default_agents_still_auto_created` — `set_running("pm", "task")` → `get("pm").state == RUNNING` (regression)
...
- [x] Run `uv run pytest` — all tests pass (943+ existing + 11 new)
- [x] Run `uv run ruff check .` — no lint errors
- [x] Run `uv run ruff format .` — no formatting issues
- [x] Verify no `DEFAULT_AGENTS` / `VALID_AGENTS` duplication was introduced (single source of truth preserved)
- [x] Verify error message format matches: `Unknown agent: '{id}'. Valid agents: ba, builder-1, builder-2, pm, reviewer, writer`
