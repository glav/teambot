...
- [x] **Task 1.1**: Write router recognition tests (Details: Lines 35-55)
...
- [x] All tests pass: `uv run pytest tests/test_repl/test_router.py tests/test_tasks/test_executor.py -v`
- [x] Artifacts: VALID_AGENTS includes "notify", PSEUDO_AGENTS constant exists
...
- [x] **Task 2.6**: Implement result storage (Details: Lines 189-195)
...
- [x] Tests pass: `uv run pytest tests/test_tasks/test_executor.py::TestNotifyPseudoAgent -v`
- [x] Artifacts: `_handle_notify()` method, truncation helper
...
- [x] **Task 3.1**: Write simple execution intercept tests (Details: Lines 197-220)
  - Test `@notify "message"` executes without SDK call
  - Test `@notify "$pm"` with $ref interpolation
  - Test background mode with `&`

- [x] **Task 3.2**: Add intercept to `_execute_simple()` (Details: Lines 222-245)
...
- [x] **Task 3.3**: Write pipeline intercept tests (Details: Lines 247-265)
  - Test `@pm plan, @notify "done", @reviewer review`
  - Test `@notify "start", @pm plan`
  - Test `@pm plan, @notify "end"`

- [x] **Task 3.4**: Add intercept to `_execute_pipeline()` (Details: Lines 267-280)
  - File: `src/teambot/tasks/executor.py`
  - Check each stage's agent_ids for "notify"
  - Handle inline without task creation

- [x] **Task 3.5**: Write multi-agent intercept tests (Details: Lines 282-290)
  - Test `@pm,notify task` syntax
  - Test notify executes with other agents

- [x] **Task 3.6**: Add intercept to `_execute_multiagent()` (Details: Lines 292-300)
  - File: `src/teambot/tasks/executor.py`
  - Handle notify separately from SDK-based agents

### Phase Gate: Phase 3 Complete When
- [x] All Phase 3 tasks marked complete
- [x] Tests pass: `uv run pytest tests/test_tasks/test_executor.py -v`
- [x] Validation: `@pm plan, @notify "done"` works in REPL
- [x] Artifacts: All executor intercepts implemented
...
- [x] **Task 4.1**: Write failure non-blocking tests (Details: Lines 302-325)
...
- [x] **Task 4.2**: Implement failure handling (Details: Lines 327-345)
...
- [x] Tests pass: failure scenarios work correctly
- [x] Validation: Notification failure doesn't break pipeline
- [x] Artifacts: try/except wrapper with logging
...
- [x] **Task 5.1**: Write legacy removal tests (Details: Lines 347-365)
  - Test `/notify "test"` returns unknown command
  - Test error message suggests using `@notify`

- [x] **Task 5.2**: Remove `/notify` command (Details: Lines 367-385)
  - File: `src/teambot/repl/commands.py`
  - Remove `"notify": self.notify` from handlers dict (Line 691)
  - Delete `notify()` method (Lines 762-828)

- [x] **Task 5.3**: Update help text (Details: Lines 387-395)
  - Remove `/notify` from /help output
  - Ensure no references to `/notify` in help

### Phase Gate: Phase 5 Complete When
- [x] All Phase 5 tasks marked complete
- [x] Tests pass: `uv run pytest tests/test_repl/test_commands.py -v`
- [x] Validation: `/notify` shows unknown command error
- [x] Artifacts: `/notify` removed, help updated
...
- [x] **Task 6.1**: Write status display tests (Details: Lines 397-420)
  - Test status panel data includes "notify" agent
  - Test model for notify is "(n/a)"
  - Location: `tests/test_ui/test_agent_state.py`

- [x] **Task 6.2**: Add "notify" to DEFAULT_AGENTS (Details: Lines 422-435)
  - File: `src/teambot/ui/agent_state.py:73`
  - Change: Add "notify" to list

- [x] **Task 6.3**: Update StatusPanel for "(n/a)" model (Details: Lines 437-450)
...
- [x] Tests pass: `uv run pytest tests/test_ui/ -v`
- [x] Validation: Status panel shows `notify (n/a) idle`
- [x] Artifacts: UI shows @notify correctly
...
- [x] Full test suite passes with 82% coverage (1412 tests)
- [x] No lint errors
- [x] Artifacts: Clean test and lint output
...
- [x] `@notify` recognized as valid agent ID
- [x] `@notify "message"` sends notification
- [x] `@notify "$pm"` interpolates agent output
- [x] Large outputs truncated to 500 chars
- [x] Notification failures don't break pipeline
- [x] Result stored for `$notify` references
- [x] `/notify` returns unknown command
- [x] Status panel shows `notify (n/a) idle`
