...
- [x] **Task 1.1**: Create `tests/test_scaffolds.py` with test class structure and fixtures
- [x] **Task 1.2**: Create `tests/test_init_scaffolds_acceptance.py` with acceptance test structure

### Phase Gate: Phase 1 Complete When
- [x] Both test files created with class skeletons
...
- [x] **Task 2.1**: Write tests for `get_scaffolds_dir()` function
  - Test: returns Path object
  - Test: scaffolds directory exists
  - Test: contains expected files (stages.yaml)
- [x] **Task 2.2**: Implement `get_scaffolds_dir()` in `src/teambot/scaffolds.py`

### Phase Gate: Phase 2 Complete When
- [x] All resource locator tests pass: `uv run pytest tests/test_scaffolds.py::TestGetScaffoldsDir -v`
...
- [x] **Task 3.1**: Write tests for `copy_scaffold_file()` function
...
- [x] **Task 3.2**: Implement `copy_scaffold_file()` function

### Phase Gate: Phase 3 Complete When
- [x] All file copier tests pass: `uv run pytest tests/test_scaffolds.py::TestCopyScaffoldFile -v`
...
- [x] **Task 4.1**: Write tests for `copy_scaffold_directory()` function
  ...
  - Test: returns CopyResult with correct status
- [x] **Task 4.2**: Implement `copy_scaffold_directory()` function

### Phase Gate: Phase 4 Complete When
- [x] All directory copier tests pass: `uv run pytest tests/test_scaffolds.py::TestCopyScaffoldDirectory -v`
...
- [x] **Task 5.1**: Write tests for `copy_all_scaffolds()` function
...
- [x] **Task 5.2**: Implement `copy_all_scaffolds()` function

### Phase Gate: Phase 5 Complete When
- [x] All orchestrator tests pass: `uv run pytest tests/test_scaffolds.py::TestCopyAllScaffolds -v`
- [x] Coverage for scaffolds.py ≥ 90% (actual: 98%)
...
- [x] **Task 6.1**: Update `pyproject.toml` with scaffold include pattern
- [x] **Task 6.2**: Verify bundle by building wheel and inspecting contents

### Phase Gate: Phase 6 Complete When
- [x] Build succeeds: `uv build`
- [x] Wheel contains scaffolds: `unzip -l dist/*.whl | grep scaffolds`
...
- [x] **Task 7.1**: Write CLI integration tests in `tests/test_cli.py`
  - Test: init copies scaffolds to new repo
  - Test: init skips existing scaffolds
  - Test: init --force overwrites scaffolds
- [x] **Task 7.2**: Integrate `copy_all_scaffolds()` into `cmd_init()` in `src/teambot/cli.py`

### Phase Gate: Phase 7 Complete When
- [x] All CLI tests pass: `uv run pytest tests/test_cli.py::TestCLIInit -v`
- [x] Existing CLI tests still pass (22/22 passing)
...
- [x] **Task 8.1**: Run and verify all acceptance tests pass
- [x] **Task 8.2**: Update documentation (README.md)

### Phase Gate: Phase 8 Complete When
- [x] All tests pass: `uv run pytest tests/test_scaffolds.py tests/test_cli.py -v`
- [x] Coverage ≥ 80% for scaffolds.py (actual: 98%)
- [x] Documentation updated with new init behavior
