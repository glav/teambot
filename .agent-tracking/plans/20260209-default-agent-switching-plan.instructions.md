...
- [x] **Task 1.1**: Write tests for `AgentRouter.set_default_agent()` and `get_config_default_agent()`
  - File: `tests/test_repl/test_router.py`
  - Add class `TestRouterDefaultAgentMutation` with 6 tests
  - Details: Lines 10-39

- [x] **Task 1.2**: Implement `set_default_agent()`, `get_config_default_agent()`, `_config_default_agent` on `AgentRouter`
...
- [x] `uv run pytest tests/test_repl/test_router.py -v` passes
- [x] `get_default_agent()` returns mutated value; `get_config_default_agent()` returns original
- [x] Invalid agent ID raises `RouterError`
...
- [x] **Task 2.1**: Write tests for `handle_use_agent()` handler function
...
- [x] **Task 2.2**: Implement `handle_use_agent()` module-level function
  - File: `src/teambot/repl/commands.py`
  - Follow `handle_model()` pattern (lines 218-277)
  - Details: Lines 117-138

- [x] **Task 2.3**: Write tests for `handle_reset_agent()` handler function
  - File: `tests/test_repl/test_commands.py`
  - Add class `TestResetAgentCommand` with 3 tests (reset, already-at-default, no-router)
  - Details: Lines 140-155

- [x] **Task 2.4**: Implement `handle_reset_agent()` module-level function
  - File: `src/teambot/repl/commands.py`
  - Details: Lines 157-162

### Phase 2 Gate
- [x] All Phase 2 tasks marked complete
- [x] `uv run pytest tests/test_repl/test_commands.py -v -k "UseAgent or ResetAgent"` passes
- [x] All 8 handler tests pass (5 use-agent + 3 reset-agent)
...
- [x] **Task 3.1**: Add `_router` parameter to `SystemCommands.__init__()` and add `set_router()` setter
  - File: `src/teambot/repl/commands.py`
  - Follow existing `set_executor()` / `set_overlay()` pattern (lines 548-562)
  - Details: Lines 170-188

- [x] **Task 3.2**: Add `use_agent()` and `reset_agent()` wrapper methods to `SystemCommands`
  - File: `src/teambot/repl/commands.py`
  - Add dispatch entries `"use-agent"` and `"reset-agent"` to handlers dict (after line 585)
  - Details: Lines 190-210

- [x] **Task 3.3**: Write dispatch integration tests
  - File: `tests/test_repl/test_commands.py`
...
- [x] `dispatch("use-agent", ["builder-1"])` returns success with correct output
- [x] Unknown commands still return error
...
- [x] **Task 4.1**: Update `handle_help()` to include `/use-agent` and `/reset-agent`
  - File: `src/teambot/repl/commands.py`
  - Add entries to main help output (lines 85-98)
  - Details: Lines 225-238

- [x] **Task 4.2**: Update `handle_status()` to show default agent info
  - File: `src/teambot/repl/commands.py`
...
- [x] **Task 4.3**: Update `SystemCommands.status()` to pass router info to `handle_status()`
  - File: `src/teambot/repl/commands.py`
  - Modify `status()` method (lines 601-614) to pass router
  - Details: Lines 262-270

- [x] **Task 4.4**: Write tests for help and status enhancements
  - File: `tests/test_repl/test_commands.py`
...
- [x] `/help` output contains `/use-agent` and `/reset-agent`
- [x] `/status` output contains "Default Agent:" line
- [x] `uv run pytest tests/test_repl/test_commands.py -v` passes
...
- [x] **Task 5.1**: Add `_default_agent` field, `set_default_agent()`, and `get_default_agent()` to `AgentStatusManager`
...
- [x] **Task 5.2**: Write tests for `AgentStatusManager` default agent tracking
...
- [x] `uv run pytest tests/test_ui/test_agent_state.py -v` passes
- [x] Listener is notified when default changes; not notified on no-op set
...
- [x] **Task 6.1**: Update `StatusPanel._format_status()` to show default agent indicator
...
- [x] **Task 6.2**: Write tests for status panel default indicator
...
- [x] `uv run pytest tests/test_ui/test_status_panel.py -v` passes
- [x] Default agent row displays indicator; non-default rows do not
...
- [x] **Task 7.1**: Wire router to `SystemCommands` in `REPLLoop.__init__()`
  - File: `src/teambot/repl/loop.py`
  - Pass router to `SystemCommands` constructor (line 58) or use `set_router()`
  - Details: Lines 368-380

- [x] **Task 7.2**: Wire router to `SystemCommands` in `TeamBotApp.__init__()`
...
- [x] **Task 7.3**: Update `TeamBotApp._handle_system_command()` to refresh status panel after `/use-agent` and `/reset-agent`
  - File: `src/teambot/ui/app.py`
  - Add post-dispatch handler (after line 312), following `/model` pattern
  - Details: Lines 400-418

- [x] **Task 7.4**: Update `TeamBotApp._get_status()` to show default agent info
  - File: `src/teambot/ui/app.py`
  - Add default agent line to status output (after line 414)
  - Details: Lines 420-432

- [x] **Task 7.5**: Write integration tests for REPL and UI wiring
...
- [x] `uv run pytest tests/test_repl/test_loop.py tests/test_ui/test_app.py -v` passes
- [x] Router accessible from SystemCommands in both modes
- [x] Status panel updates after `/use-agent` in UI mode
...
- [x] **Task 8.2**: Run linting
  - Command: `uv run ruff check . && uv run ruff format --check .`

- [x] **Task 8.3**: Manual smoke test (if applicable)
